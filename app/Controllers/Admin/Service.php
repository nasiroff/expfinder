<?php


namespace App\Controllers\Admin;


use App\Controllers\BaseController;
use App\Entities\Services;
use App\Models\CategoryModel;
use App\Models\ProductModel;
use App\Models\ServiceModel;
use CodeIgniter\Database\Exceptions\DataException;

class Service extends BaseController
{

    public function index()
    {
        return view(ADMIN_VIEW_PAGE . 'services');

    }

    public function new()
    {
        return view(ADMIN_VIEW_PAGE . "create_service");
    }


    public function show($id)
    {
        $service = new ServiceModel();
        $service = $service->select([
            'id',
            'title',
            'img',
            'description',
            'price',
            'status'
        ])->find($id)->toArray();



        return $this->response->setContentType('application/json')->setJSON($service);
    }

    public function create()
    {

        $validation = \Config\Services::validation();
        $rules = [
            'title' => [
                'rules' => 'required|min_length[10]|max_length[200]',
                'errors' => [
                    'min_length' => ' choose min length'
                ]

            ],
            'description' => 'required',
            'price' => 'required',
            'status' => 'required',
            'image' => [
                'mime_in[image,image/jpg,image/jpeg,image/gif,image/png]',
                'max_size[image,400]'
            ]
        ];
        $validation->setRules($rules);

        if ($validation->withRequest($this->request)->run()) {


            $service = new \App\Entities\Services();
            $serviceModel = new ServiceModel();
            $service->fill($this->request->getPostGet());

            if ($this->request->getFile('image')->isReadable()) {
                $imagePath = $this->moveImage();
                if ($imagePath != false) {
                    $service->img = $imagePath;
                }
            }
            $serviceModel->save($service);

            session()->setFlashdata(['success' => "Xidmət müvəfəqiyyətlə əlavə olundu"]);
            if ($this->request->getPostGet('is-stay-page') == "on") {
                session()->setFlashdata(['is-stay-page' => "on"]);
                return redirect()->back();
            }
            return redirect()->to(base_url("admin/services"));

        }
        session()->setFlashdata(['errors' => $validation->getErrors()]);
        return redirect()->back()->withInput();


    }


    public function uploadPhoto()
    {
        $preview = $config = [];
        $validated = $this->validate([
            'image' => [
                'uploaded[image]',
                'mime_in[image,image/jpg,image/jpeg,image/gif,image/png]',
                'max_size[image,400]',
            ],
        ]);
        if ($validated) {
            $file = $this->request->getFile('image');
            $fileName = $this->request->getFile('image')->getName();
            $id = $this->request->getGetPost('id');
            $serviceModel = new ServiceModel();
            $service = $serviceModel->find($id);
            if (is_null($service)) {
                return $this->response->setJSON(['message' => 'Belə bir xitmət tapılmadı'])
                    ->setStatusCode(204);
            }
            if (!is_null($service->img) && file_exists($service->img)) {
                unlink($service->img);
            }

            $imgName = $file->getRandomName();
            $path = 'uploads/images/service/';
            $imgWithPath = $path . $imgName;

            if ($file->move($path, $imgName)) {
                $preview = "/".$imgWithPath;
                $config[] = [
                    'key' => $id,
                    'caption' => $fileName,
                    'downloadUrl' => base_url($imgWithPath),
                    'url' => base_url('/admin/services/delete-photo')
                ];
                $service->img = $imgWithPath;
                $result = $serviceModel->save($service);
                if ($result) {
                    return $this->response->setJSON(['message' => 'Şəkil müvəfəqiyyətlə yükləndi'])
                        ->setJSON(['initialPreview' => $preview, 'initialPreviewConfig' => $config, 'initialPreviewAsData' => true])
                        ->setStatusCode(200);
                }
            }
        }
        return $this->response->setStatusCode(409, 'Şəkilin tipi jpg, jpeg, gif və png formatında olamalı, ölçüsü isə 400KB-dan az olmalıdır');
    }

    public function deletePhoto()
    {
        $request = $this->request;
        $id = $request->getPostGet('id');
        $image = ltrim($request->getPostGet('image'), "/");

        $response = [];
        $serviceModel = new ServiceModel();
        $service = $serviceModel->find($id);
        if (is_null($service)) {
            return $this->response
                ->setStatusCode(204, 'error')
                ->setJSON(['message' => "Belə bir xidmət tapılmadı"]);
        }

        if (!is_null($image) && file_exists($image) && $image == $service->img) {
            if ($serviceModel->update($id, ['img' => null])) {
                if (unlink($image)) {
                    return $this->response
                        ->setStatusCode(200, 'success')
                        ->setJSON(['message' => 'Şəkil müvəfəqiyyətlə silindi']);
                }
            }
        }
        $response = ['code' => 409, 'message' => 'Şəkil tapılmadı'];
        return $this->response
            ->setStatusCode($response['code'], 'error')
            ->setJSON(['message' => $response['message']]);


    }

    public function update($id)
    {

        $validation = \Config\Services::validation();
        helper(['form', 'url']);
        $rules = [
            'title' => [
                'rules' => 'required|min_length[10]|max_length[200]',
                'errors' => [
                    'min_length' => ' choose min length'
                ]

            ],
            'description' => 'required',
            'price' => 'required',
            'status' => 'required'
        ];
        $validation->setRules($rules);


        if ($validation->withRequest($this->request)->run()) {

            $inputs = $this->request->getRawInput();
            $serviceModel = new ServiceModel();
            try {
                $service = $serviceModel->find($id);
                if (is_null($service)) {
                    return $this->response->setStatusCode(404, 'error')->setJSON(['message' => "Belə bir xidmət tapılmadı"]);
                }
                $service->title = $inputs['title'];
                $service->description = $inputs['description'];
                $service->price = $inputs['price'];
                $service->status = $inputs['status'];
                $serviceModel->save($service);
                return $this->response->setStatusCode(200, 'success')->setJSON(['message' => 'Xidmətin məlumatları müvəffəqiyyətlə dəyişdirildi']);
            } catch (DataException $e) {
                return $this->response->setStatusCode(500, 'error')->setJSON(['message' => 'Dəyişmək üçün məlumat tapılmadı']);
            } catch (\Exception $exception) {
                return $this->response->setStatusCode(500, 'error')->setJSON(['message' => $exception->getMessage()]);
            }
        }
        return $this->response->setStatusCode(422, 'error')->setJSON(['message' => 'Daxil edilmiş məlumatlar yoxlamadan ugurla keçmədi', 'errors' => $validation->getErrors()]);
    }



    public function delete($id)
    {
        $serviceModel = new ServiceModel();
        $service = $serviceModel->find($id);
        if (!is_null($service)) {
            $serviceModel->delete($service->id);
            return $this->response->setStatusCode(200, 'success')->setJSON(['message' => 'Xidmət müvəfəqiyyətlə silindi ']);
        }
        return $this->response->setStatusCode(404, 'error')->setJSON(['message' => 'Silmək üçün belə bir xidmət tapılmadı. Xahiş olunur yenidən cəhd edin']);
    }



    public function serverSide()
    {
        $service = new ServiceModel();
        $service = $service->select([
            'id',
            'title',
            'price',
            'description',
            'status',
            ])
            ->searchInDT(isset($_GET['search']) ?  $this->request->getPostGet('search')['value'] : null)
            ->sortDataTable($this->request->getGetPost('order')[0]['column'], $this->request->getGetPost('order')[0]['dir'])
            ->limit($this->request->getGetPost('length'))
            ->offset($this->request->getGetPost('start'))
            ->getDTData($this->request->getPostGet('draw'));

        return $this->response->setJSON($service);
    }



    private function moveImage()
    {
        $file = $this->request->getFile('image');
        $imgName = $file->getRandomName();
        $path = 'uploads/images/service/';

        if ($file->move($path, $imgName)) {
            return $path . $imgName;
        }

        return false;
    }

}