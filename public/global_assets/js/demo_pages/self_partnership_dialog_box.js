...
let dt = $('.datatable-basic').DataTable({
    ...
    "processing": true,
    "serverSide": true,
    "ajax": origin + "admin/partnerships/server-side",
    "columns": [
        {"data": "id"},
        {"data": "name"},
        {"data": "title"},
        {"data": "description"},
        {
            ...
            "render": function (data, type, full, meta) {
                return '<div class="list-icons" style="display: inline; text-align: center">\n' +
                    ...
                    '                                                    <button type="button" onclick="openDialogBox(\'' + (origin + 'admin/partnerships/' + full.id) + '\')" class="dropdown-item"><i class="icon-design"></i>\n' +
                    '                                                        Bax və dəyiş</button>\n' +
                    '                                                    <button type="button" onclick="deleteRow(\'' + (origin + 'admin/partnerships/' + full.id) + '\')" class="dropdown-item"><i class="icon-trash"></i>\n' +
                    '                                                        Sil</button>' +
                    '                                                </div>\n' +
                    '                                            </div>\n' +
                    '                                        </div>';
...
function openDialogBox(url) {
    $.getJSON(url, (response, status) => {
        ...
        if (status === 'success') {
            partnershipId = response.id;

            $('#partnership_id').val(partnershipId);
            ...
            if (response.img !== null) {
                $("#partnership_img").attr("src", origin + response.img);
                $('[data-popup="lightbox"]').attr("href", origin + response.img);
                $("#image_path").val(response.img);
                $('.partnership-photo').show();
            } else {
                $('.partnership-photo').hide();
            }
            $('.file-input-ajax').fileinput({
                language: "az",
                uploadUrl: window.location.origin + "/admin/partnership/upload-photo",
                deleteUrl: origin + "/admin/partnership/delete-photo",
                ...
                uploadExtraData: function (previewId, index) {
                    let data = {
                        id: $('#partnership_id').val(),
                    };
                    return data;
                }
            }).on('fileuploaded', function (event, previewId, index, fileId) {
                $('.partnership-photo').show(500);
                $("#partnership_img").attr("src", previewId.response.initialPreview);
...
$.extend(buttons, [
    {
        ...
        click: function () {
            confirmation(function () {
                updateRow(origin + "admin/partnerships/" + partnershipId)
            }, {
                title: "Əminsinizmi?",
    ...
    },
    {
        ...
        click: function () {
            ...
            deleteRow(origin + "admin/partnerships/" + partnershipId);
...
let init = function () {
    $("#delete-img").on('click', function (e) {
        confirmation(function () {
            $.ajax({
                url: origin + "admin/partnerships/delete-photo",
                type: "POST",
                data: {
                    image: $("#image_path").val(),
                    id: $('#partnership_id').val()
                ...
                success: function (data, status, dd) {
                    if (status === "success") {
                        swal(data.message, {
                            ...
                            $("#partnership_img").attr('src', "");
                            $('[data-popup="lightbox"]').attr("href", "");
                            $('.partnership-photo').hide(500);
                        });
                    } else {
                        swal(data.message, {
                            icon: "error",
