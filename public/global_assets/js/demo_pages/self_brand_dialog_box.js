...
let dt = $('.datatable-basic').DataTable({
  ...
  "processing": true,
  "serverSide": true,
  "ajax": origin + "admin/brands/server-side",
  "columns": [
    {"data": "id"},
    {"data": "name"},
    {
      ...
      "render": function (data, type, full, meta) {
        return '<div class="list-icons" style="display: inline; text-align: center">\n' +
          ...
          '                                                    <button type="button" onclick="openDialogBox(\'' + (origin + 'admin/brands/' + full.id) + '\')" class="dropdown-item"><i class="icon-design"></i>\n' +
          '                                                        Bax və dəyiş</button>\n' +
          '                                                    <button type="button" onclick="deleteRow(\'' + (origin + 'admin/brands/' + full.id) + '\')" class="dropdown-item"><i class="icon-trash"></i>\n' +
          '                                                        Sil</button>' +
          '                                                </div>\n' +
          '                                            </div>\n' +
          '                                        </div>';
...
function openDialogBox(url) {
  $.getJSON(url, (response, status) => {
    ...
    if (status === 'success') {
      ...
      if (typeof CKEDITOR.instances['about'] !== 'undefined') {
        CKEDITOR.instances['about'].destroy();
      }
      ...
      if (response.img !== null) {
        $("#brand_img").attr("src", origin + response.img);
      ...
      $('.file-input-ajax').fileinput({
        language: "az",
        uploadUrl: window.location.origin + "/admin/brands/upload-photo",
        deleteUrl: origin + "/admin/brands/delete-photo",
        ...
        $('.brand-photo').show(500);
        $("#brand_img").attr("src", previewId.response.initialPreview);
...
$.extend(buttons, [
  {
    ...
    click: function () {
      confirmation(function () {
        updateRow(origin + "admin/brands/" + brandId)
      }, {
        title: "Əminsinizmi?",
  ...
  },
  {
    ...
    click: function () {
      ...
      deleteRow(origin + "admin/brands/" + brandId);
...
let init = function () {
  $("#delete-img").on('click', function (e) {
    confirmation(function () {
      $.ajax({
        url: origin + "admin/brands/delete-photo",
        type: "POST",
        data: {
          image: $("#image_path").val(),
          id: $('#brand_id').val()
